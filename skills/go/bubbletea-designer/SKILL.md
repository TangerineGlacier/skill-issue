...
Load these on demand rather than reading them up front:

- [references/design-patterns.md](references/design-patterns.md) — the six
  structural patterns (single-view, state machine, composable, master-detail,
  form flow, progress tracker) and the Lipgloss layout idioms.
- [references/bubbletea-components-guide.md](references/bubbletea-components-guide.md) —
  what each Bubbles component is for and its key methods.
- [references/architecture-best-practices.md](references/architecture-best-practices.md) —
  model/update/view organisation, performance, and the common pitfalls.
